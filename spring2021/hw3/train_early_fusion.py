  ...
  for line in fread.readlines()[1:]:
    ...
    if os.path.exists(feat_filepath) and os.path.exists(feat_filepath2):
      ...
      if tmp_feat.shape != (1024,):
          continue
      feat_list2.append(tmp_feat / np.linalg.norm(tmp_feat))
    
      tmp_feat = np.genfromtxt(feat_filepath, delimiter=";", dtype="float")
      feat_list.append(tmp_feat / np.linalg.norm(tmp_feat))
